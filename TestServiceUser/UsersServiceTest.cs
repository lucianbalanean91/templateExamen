...
namespace Tests
{
    public class UsersServiceTest
    {
        ...
        public void ValidUsernameAndPasswordShouldLoginSuccessfully()
        {
            ...
            var options = new DbContextOptionsBuilder<UsersDbContext>()
              .UseInMemoryDatabase(databaseName: nameof(ValidUsernameAndPasswordShouldLoginSuccessfully))
              .Options;
        ...
        public void Upsert()
        {
            ...
            using (var context = new UsersDbContext(options))
            {
                ...
                UserGetModel update = usersService.Upsert(1, toUpdateWith, user2);

                Assert.AreNotEqual(update.Username, firstAdded.Username);
                Assert.AreNotEqual(update.Email, firstAdded.Email);
                Assert.AreNotEqual(update.UserRole, RoleConstants.ADMIN);
            }
        }
